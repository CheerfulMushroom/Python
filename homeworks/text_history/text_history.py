...
def test_replace():
    """Doesn't affect anything, can be deleted. Made for showing optimisation"""
    test_replace = TextHistory('1234567890')
    print(test_replace.text)
    test_replace.replace('abcde', 1)
    print(test_replace.text)
    test_replace.replace('XYZ', 2)
    print(test_replace.text)
    test_replace.replace('QWE', 4)
    print(test_replace.text)
    print(test_replace.get_actions())
    print()


def test_delete():
    """Doesn't affect anything, can be deleted. Made for showing optimisation"""
    test_delete = TextHistory('Cakes')
    print(test_delete.text)
    test_delete.delete(2, 1)
    print(test_delete.text)
    test_delete.delete(1, 2)
    print(test_delete.text)
    test_delete.delete(0, 1)
    print(test_delete.text)
    print(test_delete.get_actions())
    print()


def test_insert():
    """Doesn't affect anything, can be deleted. Made for showing optimisation"""
    test_insert = TextHistory('abcdef')
    print(test_insert.text)
    test_insert.insert('XYZ', 2)
    print(test_insert.text)
    test_insert.insert('123', 3)
    print(test_insert.text)
    print(test_insert.get_actions())
    print()


def test_all():
    """Doesn't affect anything, can be deleted. Made for showing optimisation"""
    h = TextHistory('GLaDOS is very good. Cake exists!!!')
    print(h.text)
    h.delete(9, 5)
    print(h.text)
    h.delete(9, 6)
    print(h.text)
    h.insert(' dangerous!', 9)
    print(h.text)
    h.insert(' extremely', 9)
    print(h.text)
    h.replace(' is lie', 35)
    print(h.text)
    h.replace('is a lie!!!', 36)
    print(h.text)
    print(h.get_actions())
    print()
